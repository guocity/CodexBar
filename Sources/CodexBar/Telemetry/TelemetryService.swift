...
struct KeychainTelemetryTokenStore {
    ...
    func storeToken(_ token: String?) {
        ...
        if updateStatus == errSecItemNotFound {
            ...
            _ = KeychainSecurity.add(addQuery as CFDictionary, nil)
        }
    }
...
final class TelemetryService {
    ...
    private let serverURLOverride: String?
    private let usesInjectedToken: Bool
    private var injectedToken: String?

    init(
        enabledOverride: Bool? = nil,
        serverURLOverride: String? = nil,
        injectedToken: String? = nil,
        sessionFactory: @escaping @Sendable () -> URLSession = TelemetryService.makeSession)
    {
        self.enabledOverride = enabledOverride
        self.serverURLOverride = serverURLOverride
        self.usesInjectedToken = injectedToken != nil
        self.injectedToken = injectedToken
    ...
    var isEnabled: Bool {
        self.enabledOverride ?? UserDefaults.standard.bool(forKey: TelemetryConstants.DefaultsKey.enabled)
    }

    var serverURLString: String {
        let stored = self.serverURLOverride
            ?? UserDefaults.standard.string(forKey: TelemetryConstants.DefaultsKey.serverURL)
        let trimmed = stored?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? TelemetryConstants.defaultServerURL : trimmed
    }

    func loadToken() -> String? {
        if self.usesInjectedToken { return self.injectedToken }
        return self.tokenStore.loadToken()
    }

    func storeToken(_ token: String?) {
        if self.usesInjectedToken {
